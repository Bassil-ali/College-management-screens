...
class datetime extends \DateTime
{
     ...
     */
    public Function createFromHijri($year, $month, $day, \DateTimeZone $timezone = null, $langcode = null, $hijriCalendar = null)
    {
...
class Calendar
{
    ...
    public static function HijriToGregorian2($date, $delimiter = '-')
    {
        $cal = new Calendar;
        ...
        if (is_array($parts)) {
            if (count($parts) == 3) {
                $result = $cal->HijriToGregorian($parts[0], $parts[1], $parts[2]);
            } else {
                return sprintf('0000%s00%s00', $delimiter, $delimiter);
            }
        }

        if (is_array($result)) {
            if (count($result) == 3) {
                return Carbon::createFromDate($result['y'], $result['m'], $result['d']);
            } else {
                return sprintf('0000%s00%s00', $delimiter, $delimiter);
            }
        }
    ...
    public static function GregorianToHijri2($date, $format = 'Y-m-d', $tolerance = null)
    {
        $cal = new Calendar;
        $parts = explode('-', $date);
        if (!isset($tolerance)) {
            $calendar = \App\HijriCalendar::where('start', '<', $date)->where('end', '>', $date)->first();
            $tolerance = isset($calendar) ? $calendar->tolerance : 0;
            // logger($tolerance);
        }

        if (is_array($parts)) {
            if (count($parts) == 3) {
                $result = $cal->GregorianToHijri($parts[0], $parts[1], $parts[2]);
            } else {
                return '0000-00-00';
            }
        }
        ...
        if (is_array($result)) {
            if (count($result) == 3) {
                $dt = Carbon::createFromDate($result['y'], $result['m'], $result['d']);
                return $dt->addDays($tolerance)->format($format);
            } else {
                return '0000-00-00';
            }
        }
    ...
    public static function today($delimiter = '-')
    {
        ...
        $result = $cal->GregorianToHijri($now->year, $now->month, $now->day);
        $y = number_format($result['y'], 0, '', '');
        $m = number_format($result['m'], 0, '', '');
        $d = number_format($result['d'], 0, '', '');
